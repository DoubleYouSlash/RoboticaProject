        ...
        if (c != null && c.size() > 0) {
            for (int i = 0; i < c.size(); i++) {
                ...
                if (child.getChildren().size() > 0) {
                    Person[] ps = new Person[NUM_OF_PARENTS];
                    ArrayList<Person> cb = new ArrayList<>();
                    for (int j = 0; j < NUM_OF_PARENTS; j++) {
                        if (child.getGender().equals(Person.getMaleGender())) {
                            ps[0] = child;
                            ps[1] = child.getSpouse();
                        } else {
                            ps[1] = child;
                            ps[0] = child.getSpouse();
                        }
                    }
                    for (int j = 0; j < child.getChildren().size(); j++) {
                        cb.add(child.getChildren().get(j));
                    }
                    childBranches.add(new Branch(ps, cb));
                } else {
